...
class CityFeeds extends Component {
  constructor(props) {
    super(props);
    this.state = {
      feeds: [],
  ...
  getPosts = async () => {
    ...
    this.unsub = firestore()
      .collection('Posts')
      .where('cityId', '==', this.props.cityId)
    .where('tab', '==',   this.props.tab)
    .where('subcategory', '==', this.props.subcategory)
      .orderBy('date', 'desc')
      .limit(50)
      .onSnapshot(
        (snap) => {
...
export default connect(mapStateToProps, mapDispachToProps)(CityFeeds);
