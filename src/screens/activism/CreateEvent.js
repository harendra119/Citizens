...
const CreateEvent = ({navigation, route}) => {
  ...
  const editActivism = async () => {
    ...
    const date = new Date();
    const {id} = movInfo;
    const parentId = route.params?.movementId;
    ...
    if (coverUrl && coverUrl !== '' && coverUrl !== movInfo.eventUrl) {
      try {
        const randomVar = title.split(' ')[0] + date.getTime();

        const ref2 = storage().ref(`images/event/cover/${randomVar}`);
        await ref2.putFile(coverUrl);
        coverPhoto = await ref2.getDownloadURL();
        movPayLoad['eventUrl'] = coverPhoto;
      } catch (error) {
        setShowLoader(false);
    ...
    try {
      await firestore().collection('Movements').doc(parentId).collection('Events').doc(id).update(movPayLoad);
      setShowLoader(false);
      navigation.goBack();
      setShowLoader(false);
    } catch (error) {
  ...
  const createActivism = async () => {
    ...
    const MovemntId = route.params?.movementId;
    const userName = route.params?.username;
    if (movInfo) {
      editActivism();
      return;
    ...
    if (title && userName  && start_date && end_date && MovemntId && category != 'Select Category') {
      setShowLoader(true);
      ...
      try {
        ...
        if (coverUrl) {
          ...
          const ref2 = storage().ref(`images/event/image/${randomVar}`);
          try {
            await ref2.putFile(coverUrl);
            eventImage = await ref2.getDownloadURL();
          } catch (error) {
            alert('Somthing went wrong while uploading Event Image.');
          }
        }

        let followRef = firestore().collection('Follows').doc();
        const movRef = firestore().collection('Movements').doc(MovemntId);
        const eventsRef = movRef.collection('Events').doc();
        ...
        batch.set(eventsRef, movPayLoad);
      ...
      <KeyboardAvoidingView behavior='padding' enabled style={{ flex: 1 }}>
        <View style={{height: vScale(165)}}>
          ...
          <TouchableOpacity
            ...
            <Text style={styles.buttonText}>Select Event Image</Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={() => setCoverUrl('')}
            style={{
        ...
        <Text style={{...styles.text1,marginBottom: vScale(10)}}>Categories</Text>
          <ListItem style={{marginBottom: vScale(10)}}
                ...
                <ListItem.Content>
                  <ModalDropdown
                    options={['Demonstrations and protests', 'Boycotts', 'Strikes','Letter-writing and petitions','Volunteer','Web']}
                    dropdownStyle={{
                      width: wp(70),
                      marginTop: hp(-4),
