...
const EventDetails = ({navigation, route}) => {
  ...
  const [parentId, setParentId] = useState(route.params?.parent_id || '');
  ...
  const fetchMovementDetails = useCallback(() => {
    if (!movementId || !movement) return;
    const movRef = firestore().collection('Movements').doc(parentId);
    movRef.onSnapshot(
        (snap) => {
      ...
      .onSnapshot(
        (snap) => {
          ...
          console.log('Event Data',snap.data().start_date.seconds);
          setMovement(snap.data());
          setStartDate(new Date((snap.data().start_date.seconds)*1000));
          setEndDate(new Date((snap.data().end_date.seconds)*1000))
        },
        (err) => console.log('error', err),
      );
  }, [movementId]);
    ...
    // else {
      ...
      try {
        ...
        var movementSnap = await firestore()
          ...
          .doc(parentId).collection('Events').doc(id2)
          .get();
  ...
  const unFollowMovement = async () => {
    ...
    try {
      ...
      var movementSnap = await firestore()
        .collection('Movements').doc(parentId).collection('Events').doc(id2)
        .get();
  ...
  const onSelectOptions = async (index, value) => {
    if (value == 'Edit') {
      ...
      // console.log(movement);return false;
      // navigation.navigate('CreateActivism', {movInfo: movement});
      navigation.navigate('CreateEvent', {movInfo: movement,movementId:parentId,username:movement.username});

    }
    // if (value == 'Create Event') {
    //   // console.log(movement);
    //   navigation.navigate('CreateEvent', {movInfo: movement});
    // }
    // else 
      ...
      try {
        ...
        const movRef = firestore().collection('Movements').doc(parent_id).collection('Events');
        movRef.get().then(
            (snap) => {
        ...
        navigation.navigate('ActivismDetails', {id: id})
      } catch (error) {
        setDropdownLoading(false);
        defaultAlert();
        console.log('error while follow', error);
  ...
  const renderPageHeader = () => (
            ...
            <View style={styles.dropDown}>
              {dropdownLoading ? (
                ...
                <ModalDropdown
                  ...
                  dropdownStyle={{
                    width: scale(100),
                    height: Platform.OS == 'ios' ? vScale(50) : vScale(60),
                    marginLeft: scale(0),
                    padding: 0,
                    margin: 0,
                    // flexWrap: 1,
      ...
      <View style={styles.actions}>
        <View style={styles.row}>
          <Icon
              ...
              color="#878585"
            />
          <Text style={styles.text1}>{start_date.toLocaleString()}</Text>
        </View>
        ...
        <View style={styles.row}>
          <Icon
              ...
              color="#878585"
            />
          <Text style={styles.text1}>{end_date.toLocaleString()}</Text>
        </View>
        ...
        <View style={styles.row}>
          ...
          <Text style={styles.text1}>
            {renderCount(movement?.followedCount || 0)} Attending
          </Text>
        </View>
        {/* <View style={styles.row}>
          <Icon
    ...
    color: '#878585',
      ...
      <View style={{
        ...
        {/* <TouchableOpacity
            ...
            {followDetails.isFollowed ? (
                ...
                onPress={unFollowMovement}>
                  <Text style={{color: '#fff'}}>Attending</Text>
                </TouchableOpacity>
              ) : (
                <TouchableOpacity
                  style={styles.newButton}
                  ...
                  {/* <Icon
                    ...
                    color="#1e2348"
                  /> */}
                  <Text style={{color: '#fff'}}>Attend</Text>
      ...
      {/* </View> */}
      {/* {(followDetails.isFollowed && allowPost) && (
        <TouchableOpacity
